...
Then('I should be able to search and find the patient {string} in the registration module', (patient) => {
  cy.get('a[id="bahmni.registration"]').click()
    .get('input[id="name"]').type(patient)
    .get('div[class="reg-srch-btn"]').click()
    .get('table').should('contain', patient);
});