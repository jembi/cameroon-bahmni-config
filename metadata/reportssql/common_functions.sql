...
-- patientAgeWhenTestedForHivProgramIsBetween

DROP FUNCTION IF EXISTS patientAgeWhenTestedForHivProgramIsBetween;  

DELIMITER $$ 
CREATE FUNCTION patientAgeWhenTestedForHivProgramIsBetween(
    p_patientId INT(11),
    p_startAge INT(11),
    p_endAge INT(11),
    p_includeEndAge TINYINT(1)) RETURNS TINYINT(1) 
...
BEGIN 
    DECLARE result TINYINT(1) DEFAULT 0;
    DECLARE dateOfTheHivTest DATETIME DEFAULT getObsDatetimeValueInSection(p_patientId, "c6c08cdc-18dc-4f42-809c-959621bc9a6c", "b70dfca0-db21-4533-8c08-4626ff0de265");
    SELECT  
        IF (p_includeEndAge, 
            timestampdiff(YEAR, p.birthdate, dateOfTheHivTest) BETWEEN p_startAge AND p_endAge, 
            timestampdiff(YEAR, p.birthdate, dateOfTheHivTest) >= p_startAge
                AND timestampdiff(YEAR, p.birthdate, dateOfTheHivTest) < p_endAge
        ) INTO result  
    FROM person p 
