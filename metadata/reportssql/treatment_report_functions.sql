...
DROP FUNCTION IF EXISTS TREATMENT_Indicator7e;

DELIMITER $$
CREATE FUNCTION TREATMENT_Indicator7e(
    p_startDate DATE,
    p_endDate DATE,
    p_startAge INT(11),
    p_endAge INT (11),
...
WHERE
    ...
    patientHasStartedARVTreatmentBefore(pat.patient_id, p_startDate) AND
    (
        patientWasOnARVTreatmentByDate(pat.patient_id, p_endDate) OR
        patientPickedARVDrugDuringReportingPeriod(pat.patient_id, p_startDate, p_endDate)
    ) AND
    patientScreenedForTBDuringReportingPeriod(pat.patient_id, p_startDate, p_endDate, "Positive") AND
    patientAgeWhenRegisteredForHivProgramIsBetween(pat.patient_id, p_startAge, p_endAge, p_includeEndAge);
