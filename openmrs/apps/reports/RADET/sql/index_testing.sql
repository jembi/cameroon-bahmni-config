SELECT getPatientIdentifier(p.patient_id) as "Contacts (Contact nr/Child nr) ",
	getPatientIndexTestingDateOffered(getFirstIndexID(p.patient_id)) as "Date of Contact elicitation from Index/ Date de sollicitation du contact à partir de l'index",
	...
	getIndexType(p.patient_id) as "Type of Index Case/ type de cas index",
	...
	getHIVTestDate(p.patient_id, "#startDate#", "#endDate#") as "Actual date of visit for HTS/ Date réelle de visite pour HTS (EID Sample Collected date/Échantillon EID Date de collecte)  (dd-MMM-yyyy)",
	...
	getDaysBetweenHIVPosAndEnrollment(getFirstIndexID(p.patient_id)) as "Time to Enrolment (days)",
	getDaysBetweenHIVPosAndART(getFirstIndexID(p.patient_id)) as "Time to ART Initiation (days)",
	getPatientIndexTestingDateOffered(getFirstIndexID(p.patient_id)) as "Date offered ICT / Date offerte ICT",
	getPatientIndexTestingDateAccepted(getFirstIndexID(p.patient_id)) as "Date accepted ICT / Date d'acceptation",
	...
	"N/A" as "APS"
	FROM patient p
		WHERE patientIsIndex(p.patient_id) OR patientIsContact(p.patient_id);