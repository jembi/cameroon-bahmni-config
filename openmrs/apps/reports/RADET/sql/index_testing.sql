SELECT getPatientIdentifier(p.patient_id) as "Contacts (Contact nr/Child nr) ",
	...
	getRadetPatientGender(p.patient_id) as "Sex",
	getPatientARTNumber(getFirstIndexID(p.patient_id)) as "Index Case Code (ART Code/ Testing Code*)",
	getRadetIndexType(getFirstIndexID(p.patient_id)) as "Type of Index Case/ type de cas index",
	getRadetFirstIndexRelationship(p.patient_id) as "Index Classification / Classification de l'indice",
	getRadetPointOfEntry(p.patient_id) as "HTS Entry/ entrée  (Testing Modality/ modalités de test)",
	...
	getRadetDefaulterNotificationMethod(p.patient_id) as "Method of contact notification/ Méthode de notification de contact",
	getRadetDefaulterNotificationOutcome(p.patient_id) as "Outcomes of notifictaion / Résultats de la notification",
	...
	getRadetTestedLocation(p.patient_id) as "Location of testing/ emplacement des tests",
